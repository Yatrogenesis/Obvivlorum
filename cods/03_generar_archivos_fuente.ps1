...
# Version corregida usando archivos temporales
...
# Crear holomem.c usando archivo temporal
...
# Escribir a archivo temporal y copiar via WSL
$tempFile = "$env:TEMP\holomem_temp.c"
$holomemContent | Out-File -FilePath $tempFile -Encoding UTF8 -NoNewline

# Copiar via WSL usando pipeline
Get-Content $tempFile -Raw | wsl -d $distro bash -c "cat > ~/obvlivorum_simbiosis/holomem/holomem.c"
...
$tempFile2 = "$env:TEMP\holomem_util_temp.c"
$utilContent | Out-File -FilePath $tempFile2 -Encoding UTF8 -NoNewline
Get-Content $tempFile2 -Raw | wsl -d $distro bash -c "cat > ~/obvlivorum_simbiosis/holomem/holomem-util.c"
...
$tempFile3 = "$env:TEMP\makefile_temp"
$makefileContent | Out-File -FilePath $tempFile3 -Encoding UTF8 -NoNewline
Get-Content $tempFile3 -Raw | wsl -d $distro bash -c "cat > ~/obvlivorum_simbiosis/holomem/Makefile"
...
# Verificar que los archivos no estan vacios
Write-Host ""
Write-Host "Verificando tamaños de archivos..." -ForegroundColor Yellow
$sizes = wsl -d $distro -- bash -c "ls -lh ~/obvlivorum_simbiosis/holomem/*.c ~/obvlivorum_simbiosis/holomem/Makefile 2>/dev/null || echo 'Algunos archivos faltan'"
Write-Host $sizes
