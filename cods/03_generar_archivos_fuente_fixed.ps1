...
# Version corregida con creacion de archivos mejorada
...
# Crear holomem.c usando metodo por partes
Write-Host "Creando holomem.c..." -ForegroundColor Cyan

# Crear archivo vacio primero
wsl -d $distro -- bash -c "touch ~/obvlivorum_simbiosis/holomem/holomem.c"

# Escribir contenido linea por linea para evitar problemas de parsing
$holomemLines = @(
...
# Escribir cada linea al archivo
foreach ($line in $holomemLines) {
    $escapedLine = $line -replace '"', '\"'
    wsl -d $distro -- bash -c "echo '$escapedLine' >> ~/obvlivorum_simbiosis/holomem/holomem.c"
}

Write-Host "Creando holomem-util.c..." -ForegroundColor Cyan

# Crear holomem-util.c usando el mismo metodo
$utilLines = @(
    '#include <stdio.h>',
    '#include <stdlib.h>',
    '#include <string.h>',
...
wsl -d $distro -- bash -c "rm -f ~/obvlivorum_simbiosis/holomem/holomem-util.c"

# Escribir cada linea
foreach ($line in $utilLines) {
    $escapedLine = $line -replace '"', '\"'
    wsl -d $distro -- bash -c "echo '$escapedLine' >> ~/obvlivorum_simbiosis/holomem/holomem-util.c"
}

Write-Host "Creando Makefile..." -ForegroundColor Cyan

# Crear Makefile (mas simple)
wsl -d $distro -- bash -c "cat > ~/obvlivorum_simbiosis/holomem/Makefile << 'EOF'
obj-m := holomem.o
